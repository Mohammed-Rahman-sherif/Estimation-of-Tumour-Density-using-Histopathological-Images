...
train_gen = DirectoryDataGenerator(base_directories, labels = tr_label, shuffle=True)
val_gen = DirectoryDataGenerator(val_directories, labels = vl_label, augmentor=False)
resnet_model.compile(optimizer=tf.keras.optimizers.Adam(lr=1e-6),loss='mean_squared_error', metrics=["MeanAbsoluteError", "MeanAbsolutePercentageError", "MeanSquaredError", "RootMeanSquaredError", "MeanSquaredLogarithmicError", "CosineSimilarity"])#standard regression metrics 

#print('#################', train_gen[1])# {issue here.. have a look!!!}

callbacks = [
    keras.callbacks.CSVLogger('C:\\Users\\smart\\Documents\\Part Time - EHU\\Workspace\\NuCLS\\Ep100\\LR_0_000006_Dropout_05\\Log_Files\\Res50\\log0.csv', separator = ',', append = True),
    keras.callbacks.ModelCheckpoint("C:\\Users\\smart\\Documents\\Part Time - EHU\\Workspace\\NuCLS\\Ep100\\LR_0_000006_Dropout_05\\Models\\Res50\\model0.{epoch:02d}-{val_loss:.2f}.h5", save_best_only=True)
]

history = resnet_model.fit(train_gen, validation_data=val_gen, epochs=100, callbacks = callbacks)
...
plt.xlabel('val_mean_squared_error')
...
plt.xlabel('val_root_mean_squared_error')
...
plt.xlabel('val_mean_squared_logarithmic_error')
...
plt.xlabel('val_cosine_similarity')
plt.legend(['Cos', 'Val_Cos'])
plt.show()