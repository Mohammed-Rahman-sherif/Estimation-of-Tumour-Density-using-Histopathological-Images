...
def build_model():
    ...
    pretrained_model= tf.keras.applications.ResNet50(include_top=False,
                   ...
                   weights='imagenet')
    for i, layer in enumerate(pretrained_model.layers[0:143]):
        print(i, layer.name)
...
resnet_model.compile(optimizer=tf.keras.optimizers.Adam(lr=0.001),loss='mean_squared_error', metrics=["MeanAbsoluteError", "MeanAbsolutePercentageError"])

#print('#################', train_gen[1])# {issue here.. have a look!!!}

callbacks = [
    keras.callbacks.CSVLogger('C:\\Users\\smart\\Documents\\Part Time - EHU\\Workspace\\NuCLS\\Log_Files\\Res50\\log0.csv', separator = ',', append = True),
    keras.callbacks.ModelCheckpoint("C:\\Users\\smart\\Documents\\Part Time - EHU\\Workspace\\NuCLS\\Models\\Res50\\model0.{epoch:02d}-{val_loss:.2f}.h5", save_best_only=True)
]

history = resnet_model.fit(train_gen, validation_data=val_gen, epochs=25, callbacks=[callbacks])
